...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if(intval(mysqli_num_rows($result)) == 1) {
    ...
    if($check !== false) {
      ...
      if ($_FILES["paymentPhoto"]["size"] > 1000000) {
        $uploadOk = 0;
        $_SESSION['file_upload']['err_file_size'] = "The file you uploaded has exceeded the capacity of 1MB";
        header('location: ../reservation/view_reservation.php');
      } 
      ...
      if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["paymentPhoto"]["tmp_name"], $target_file)) {
          ...
          $insertQuery = "INSERT INTO file_reservation (reservation_id, file_name, date_created) VALUES ('$reservation_id', '$target_file', NOW())";
